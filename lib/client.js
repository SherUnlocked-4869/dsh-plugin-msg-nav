window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      ".dsnv-panel{position:fixed;z-index:90;box-sizing:border-box;width:256px;overflow:hidden;background:var(--dsw-specific-menu, var(--dsw-alias-bg-overlay, #2C2C2E));border:1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.08));border-radius:14px;box-shadow:var(--dsw-shadow-lv3, 0 0 1px 0 rgba(0,0,0,.2), 0 12px 32px 0 rgba(0,0,0,.08));transform-origin:right center;animation:dsnv-panel-in .18s ease-out}",
      ...
      ".dsnv-panel-row{position:absolute;left:0;right:0;height:20px;box-sizing:border-box;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:0 10px 0 16px;border:0;background:transparent;cursor:pointer;font-size:13px;line-height:20px;color:var(--dsw-alias-label-tertiary, rgba(255,255,255,.45));white-space:nowrap}",
      ...
      return function NavRail(props) {
        ...
        return React.createElement('div', {
          ...
          railHot && shown ? React.createElement('div', {
            ...
            style: { right: rightPx + 'px', top: (topPx - 8) + 'px', height: (railH + 16) + 'px' },
