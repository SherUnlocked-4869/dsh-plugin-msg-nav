window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      ".dsnv-zoom{position:absolute;top:0;left:50%;width:34px;margin-left:-17px;transform-origin:top left;transition:transform .18s ease-out}",
    ...
    function makeRail(ctx) {
      ...
      return function NavRail(props) {
        ...
        return React.createElement('div', {
          ...
          role: 'navigation',
          'aria-label': '消息导航',
          style: { right: rightPx + 'px', top: (railHot ? topPx - railH * 0.15 : topPx) + 'px', height: (railHot ? railH * 1.3 : railH) + 'px' },
