window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function makeRail(ctx) {
      ...
      // 滚动侦测节流：scroll 事件只置调度标记，每帧最多跑一次 spy（rAF 合帧）。
      let spyPending = false
      function scheduleSpy() {
        if (spyPending || winEl === null) return
        spyPending = true
      ...
      function unwire() {
        if (wiredScroller !== null && winEl !== null) {
          ...
          if (rowsObserver !== null) rowsObserver.disconnect()
        }
      ...
      function ensureWired() {
        ...
        if (wiredScroller !== sc) {
          ...
          if (rowsObserver === null && typeof win.MutationObserver === 'function') {
            rowsObserver = new win.MutationObserver(onRowsMutated)
          }
          if (rowsObserver !== null) {
            rowsObserver.disconnect()
            rowsObserver.observe(sc, { childList: true, subtree: true })
          }
          rowsDirty = true
          wiredScroller = sc
          if (latest.measure) latest.measure()
      ...
      //#region keyboard prev/next jump（Alt+↑/↓ 在相邻用户消息间跳转）
      // 与点击跳转同走 jumpTo（按需加载 → 平滑落位 → 高亮横线 → 列表回中）。
      // 焦点在已有草稿的输入框/可编辑区时不触发——用户正在打字，不抢视口；
      // 空输入框（如刚发送完、焦点尚在输入框）仍可导航。仅在确定要跳转时
      // preventDefault，保留 macOS Option+↑/↓ 等原生行为。
      function editableBlocker(el) {
        ...
        if (el.isContentEditable === true) return (el.textContent || '').trim() !== ''
        return false
      }

      let kbJumpIdx = -1
      let kbJumpAt = 0
      // 跳转代际号：每次 jumpTo 递增。看门狗与落位只认最新一代——
      // 连续跳转（快捷键连按、快速点击）时，旧跳转被新跳转取消会让
      // 旧看门狗误判"滚动被干扰"而把视口夺回旧目标，必须使其失效。
      let jumpGen = 0

      // 键盘导航锚点：视口顶缘 + 落位偏移（与 jumpTo 的落位公式一致）以上
      // 最近的一条已渲染用户消息。跳转落位后目标行恰好成为锚点，向上/向下
      // 严格逐条推进。不用 scrollspy 的 33% 阅读线——近距离消息对会同时落
      // 在该线之上，导致 Alt+↑ 以同一条为基准原地打转。
      function kbAnchorIdx() {
        if (scrollerEl === null) return -1
        try {
          const srect = scrollerEl.getBoundingClientRect()
          const edge = Math.min(160, Math.max(80, Math.round(srect.height * 0.25)))
          const line = srect.top + edge + 12
          let best = -1
          for (let i = 0; i < currentUsers.length; i++) {
            const u = currentUsers[i]
            const row = u.key !== null && u.key !== undefined ? rowFor(u.key) : null
            if (row === null || row.isConnected === false) continue
      ...
      function onKeyDown(event) {
        if (event.altKey !== true || event.ctrlKey === true || event.metaKey === true || event.shiftKey === true) return
        if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp') return
        if (editableBlocker(event.target)) return
        if (railEl === null) return
        if (scrollerEl === null || scrollerEl.isConnected === false) return
        const jump = latest.jump
        if (typeof jump !== 'function') return
        // 连按（含按住重复）时以上一次快捷键目标为基准推进，避免平滑滚动
        // 途中锚点的中间读数把连按打回近处；离开 1.2s 窗口后回到实时锚点。
        const now = Date.now()
        const base = kbJumpIdx >= 0 && now - kbJumpAt < 1200 ? kbJumpIdx : kbAnchorIdx()
        let idx = base
        if (event.key === 'ArrowDown') idx = base < 0 ? 0 : base + 1
        else idx = base - 1
        if (idx < 0 || idx > currentUsers.length - 1) return
      ...
      ctx.effect(() => () => {
        try { ctx.clearInterval(iv) } catch (error) { /* noop */ }
        try { ctx.clearTimeout(t0) } catch (error) { /* noop */ }
        unwire()
        clearRing()
        stopLoadAll()
      ...
      return function NavRail(props) {
        ...
        const scrollspy = () => {
          ...
          try {
            ...
            // 节流：行集合未变且滚动位置未变时直接返回（tick 空转路径近乎零成本）
            if (!rowsDirty && top === lastSpyTop) return
            lastSpyTop = top
            const srect = scrollerEl.getBoundingClientRect()
            const line = srect.top + srect.height * 0.33
            // 行缓存只在脏时重建（MutationObserver 置脏），不再每个滚动事件全子树扫描
            if (rowsDirty) {
              rowsDirty = false
              rowsCache = {}
            ...
            for (let i = 0; i < currentUsers.length; i++) {
              ...
              if (row === undefined || row.isConnected === false) continue
              found++
        ...
        // 窗口化：只创建可视窗口内的节点元素。overflow:hidden + translateY
        // 本来就把窗口外的短横线/面板行裁掉了，全量创建只是让打开会话的
        // 首次挂载和每次滚动重渲染白白 diff O(N) 个元素——长会话卡顿主因。
        let visFrom = Math.max(0, Math.floor(offset / GAP) - 4)
        let visTo = Math.min(N - 1, Math.ceil((offset + railH) / GAP) + 4)
        if (active >= 0) {
          if (active < visFrom) visFrom = active
          if (active > visTo) visTo = active
        }
        const dotNodes = []
        if (shown) {
          for (let i = visFrom; i <= visTo; i++) {
            const u = users[i]
            dotNodes.push(React.createElement('button', {
        ...
        const panelH = Math.min(N, MAX_VIS) * 24 + 16
        let panelFrom = Math.floor((offset * 1.2 - 56) / 24)
        let panelTo = Math.ceil((offset * 1.2 + panelH + 40) / 24)
        if (panelFrom < 0) panelFrom = 0
        if (panelTo > N - 1) panelTo = N - 1
        if (active >= 0) {
          if (active < panelFrom) panelFrom = active
          if (active > panelTo) panelTo = active
        }
        const panelRowNodes = []
        if (railHot && shown) {
          for (let i = panelFrom; i <= panelTo; i++) {
            const u = users[i]
            panelRowNodes.push(React.createElement('button', {
              key: u.key !== null && u.key !== undefined ? u.key : (u.id !== undefined ? 'id-' + u.id : 'proj-' + i),
              type: 'button',
        ...
        const jumpTo = (i) => {
          ...
          void (async () => {
            ...
            if (row !== null && row.isConnected === false) row = null
            if (row === null) {
              let face = null
              ...
              if (face !== null) {
                if (u.key !== null && u.key !== undefined) {
                  ...
                  if (latest.spy) latest.spy()
                  let tries = 0
                  while (tries++ < 20 && (row = rowFor(u.key)) === null) {
                    if (gen !== jumpGen) return
                    await delay(60)
                  }
          ...
          ctx.setTimeout(() => {
            if (scrollerEl === null || gen !== jumpGen) return
