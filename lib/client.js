      ...
      return function NavRail(props) {
        ...
        const measure = () => {
          ...
          try {
            ...
            railH = m <= WIN ? Math.max(m * GAP, 2) : Math.min(Math.max(Math.round(srect.height * 0.6), 120), 560)
        ...
        const step = N > 1 ? Math.round(((railH - 3) / (N - 1)) * dpr) / dpr : 0
        const yOf = (i) => Math.round(i * step * dpr) / dpr
