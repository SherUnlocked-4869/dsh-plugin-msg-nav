window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      ".dsnv-zoom{transform-origin:center}",
      ".dsnv-zoom.dsnv-hot{animation:dsnv-pop .22s ease-out}",
      "@keyframes dsnv-pop{0%{transform:scale(1)}45%{transform:scale(1.18)}100%{transform:scale(1)}}",
      ...
      "@media (prefers-reduced-motion:reduce){.dsnv-list{transition:none}.dsnv-zoom.dsnv-hot{animation:none}.dsnv-dot{transition:none}.dsnv-highlight::after{animation-duration:.6s}}",
    ].join("\n");
    ...
    function makeRail(ctx) {
      ...
      return function NavRail(props) {
        ...
        return React.createElement('div', {
          ...
          onPointerLeave: () => { railHotRef.current = false; setRailHot(false) },
          onMouseLeave: () => {
            cancelHover()
